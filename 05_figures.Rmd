...
combo_target <- read_csv(here("data_frames/combo_target.csv")) %>% 
  mutate(trial_type = as.factor(trial_type),
         location = as.factor(location))
combo_time <- read_csv(here("data_frames/combo_time.csv"))


  
# growth curve
gca <- read_csv(here("data_frames/growth_curve_analysis.csv")) %>% 
  mutate(trial_type = as.factor(trial_type))
...
combo_target_des <- combo_target %>% 
  group_by(trial_type) %>% 
  summarize(mean = mean(Prop), min = min(Prop), max = max(Prop), sd = sd(Prop))

## standard visualization
ggplot(combo_target, aes(x = trial_type, y = Prop, color = location)) +
  geom_point() +
  ...
  geom_segment(aes(x = 1, y = combo_target_des %>% filter(trial_type == "Mixed") %>% pull(mean),
                                    xend = 2, yend = combo_target_des %>% filter(trial_type == "Single") %>% pull(mean))
                   , color = "black", size = 1) +
...
ggplot(combo_target, aes(x = trial_type, y = Prop)) +
...
# model 
gca.m <- lmer(Prop ~ (poly1+poly2) + trial_type + trial_type:poly1 + ((poly1+poly2) || study_id), 
               data=gca, REML=F)

## standard visualization
ggplot(gca, aes(x = Time, y = Prop, color = trial_type)) + 
  stat_summary(fun.y = "mean", size = 4, geom = "point") +
  stat_summary(fun.data = "mean_se", geom = "errorbar", aes(width = 0)) + 
  stat_summary(aes(y=fitted(gca.m)), fun=mean, geom="line", size = .75) +
  expand_limits(y=c(0.5, 1)) +
    scale_color_manual(values = c("#1f78b4", "#33a02c"), labels = c("Code-Switched", "Single-Language"), name = "Trial Type") +
    labs(x = "Time from Noun Onset (ms)", y = "Proportion Looking to Target")  +
    theme(legend.text = element_text(size = 12),
          legend.title = element_text(size = 14),
          axis.text = element_text(size = 12),
          axis.title = element_text(size = 14),
...
### Psychoshorts - translucent background, white text
ggplot(gca, aes(Time, Prop, color=trial_type)) + 
  stat_summary(fun.data=mean_se, geom="pointrange") + 
  stat_summary(aes(y=fitted(m.3.1)), fun=mean, geom="line") +
  expand_limits(y=c(0.5, 1)) +
