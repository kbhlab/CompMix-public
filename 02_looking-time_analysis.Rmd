...
selected_cols <- c("study_id", "trackloss", "time_from_zero", "trial_type", "trial_number", "trial_number_type", "trial_unique", "target_look", "distractor_look", "target",  "edu_years", "vocab_total","study_lang_dom", "lang_mix")
...
mtl_msl_keepers <- read_csv("data_frames/mtl_msl_keepers.csv") %>% 
  ...
  select(study_id, gender, age_years, study_language, prof_eng, prof_o, aoa_en, aoa_o, child_dom_lang, study_lang_dom, vocab_dom, vocab_non_dom, vocab_total, lang_mix, edu_years, testing_location, ethnicity)
...
gaze_trial_n <- response_window_clean %>% 
  ...
  summarize(trial_n = length(unique(trial_number)), .groups = "drop") %>% 
  ## filter for participants with at least 2 trials per condition
  pivot_wider(names_from = trial_type, values_from = trial_n) %>% 
 # pivot_wider(id_cols = c("study_id", "trial_type", "testing_location"), names_from = trial_type, values_from = trial_n) %>% #this line did not work for some reason
...
gaze_trial_type <- make_time_window_data(response_window_clean, 
                                             ...
                                             predictor_columns = c("trial_type", "study_lang_dom", "trial_number_type", "testing_location", "edu_years", "vocab_total", "lang_mix"),
...
gaze_target_sum <- gaze_target %>% 
  group_by(study_id, trial_type, study_lang_dom, edu_years, vocab_total, lang_mix, testing_location) %>% 
...
gaze_trial_time_gca <- make_time_sequence_data(response_window_clean,
                                           time_bin_size = 100,
                                           aois = c("target_look", "distractor_look"),
                                           predictor_columns = c("trial_type", "study_lang_dom", "testing_location", "trial_number", "edu_years", "vocab_total", "lang_mix"),
                                           summarize_by = "study_id") %>% 
